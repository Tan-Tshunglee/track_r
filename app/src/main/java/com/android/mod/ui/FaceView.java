...
public class FaceView extends View implements FocusIndicator {
    ...
    protected void onDraw(Canvas canvas) {
        if (mFaces != null && mFaces.length > 0) {
            // Prepare the matrix.
            com.android.mod.Util.prepareMatrix(mMatrix, mMirror, mDisplayOrientation, getWidth(), getHeight());
            ...
            for (int i = 0; i < mFaces.length; i++) {
                ...
                if (LOGV) com.android.mod.Util.dumpRect(mRect, "Original rect");
                mMatrix.mapRect(mRect);
                if (LOGV) com.android.mod.Util.dumpRect(mRect, "Transformed rect");
