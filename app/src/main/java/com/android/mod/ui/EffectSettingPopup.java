...
import com.android.mod.IconListPreference;
...
public class EffectSettingPopup extends AbstractSettingPopup implements
    ...
    public void initialize(IconListPreference preference) {
        ...
        if (hasSillyFaces) {
            findViewById(R.id.effect_silly_faces_title).setVisibility(VISIBLE);
            mSillyFacesGrid.setVisibility(VISIBLE);
        ...
        if (hasSillyFaces && hasBackground) {
            findViewById(R.id.effect_background_separator).setVisibility(VISIBLE);
        }

        // Initialize background replacer if it is supported.
        if (hasBackground) {
            findViewById(R.id.effect_background_title).setVisibility(VISIBLE);
            mBackgroundGrid.setVisibility(VISIBLE);
    ...
    public void setVisibility(int visibility) {
        if (visibility == VISIBLE) {
            if (getVisibility() != VISIBLE) {
                ...
                mClearEffects.setVisibility(noEffect ? GONE : VISIBLE);
            }
            reloadPreference();
        }
